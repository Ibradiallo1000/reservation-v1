import{dA as o,d5 as q,cU as M,dz as J,e4 as _,cJ as K,dv as ee,d6 as te,c$ as Q,d0 as ae,dn as d,dp as l}from"./vendor-C0BUejmG.js";import{bd as ie,Q as f}from"../index-PL5EmLWm.js";import{P as re,S as A,E as ne}from"./Input-DmBLAZ4M.js";import{b as se}from"./types-DuSYHM9o.js";import{n as de}from"./tripExecutionService-C8g4iq88.js";import"./typography-DRaXVLzj.js";import"./agencyCity-BG1SRcM5.js";import"./stopResolution-DTkwiPJH.js";function i(t){return String(t??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function le(t){const c=String(t.boardingStatus??"").toLowerCase();if(c==="boarded")return"embarqué";if(c==="no_show")return"absent";const a=String(t.statutEmbarquement??"").toLowerCase();return a==="embarqué"||a==="embarque"?"embarqué":a==="absent"?"absent":"en attente"}function F(t,c){const a=window.open("","_blank","noopener,noreferrer,width=980,height=900");if(!a)throw new Error("Impossible d'ouvrir la fenêtre d'impression.");a.document.open(),a.document.write(t),a.document.close(),a.document.title=c,a.focus(),a.print()}const ve=()=>{const{user:t,company:c}=ie(),a=(t==null?void 0:t.companyId)??null,m=(t==null?void 0:t.agencyId)??null,$=(t==null?void 0:t.uid)??null,P=Array.isArray(t==null?void 0:t.role)?t.role:t!=null&&t.role?[t.role]:[],I=P.includes("chefAgence")||P.includes("chefagence"),[E,S]=o.useState([]),[W,y]=o.useState(!0),[V,D]=o.useState(null),L=String((c==null?void 0:c.nom)??"Compagnie"),C=String((c==null?void 0:c.logoUrl)??""),j=String((t==null?void 0:t.agencyNom)??(t==null?void 0:t.agencyName)??m??""),H=o.useCallback(async e=>{const r=String(e.departure??e.departureCity??e.routeDeparture??"").trim(),u=String(e.arrival??e.arrivalCity??e.routeArrival??"").trim(),g=String(e.time??e.departureTime??"").trim(),n=String(e.date??"").trim(),s=String(e.vehicleId??"").trim(),h=String(e.weeklyTripId??"").trim();let x="—";if(s)try{const b=await q(M(f,`companies/${a}/fleetVehicles/${s}`));b.exists()?x=String(b.data().plateNumber??"").trim()||s:x=s}catch{x=s}let B="—",O="—";if(m&&h&&n&&g){const b=`${h}_${n}_${g}`;try{const v=await q(M(f,`companies/${a}/agences/${m}/tripAssignments/${b}`));if(v.exists()){const p=v.data();B=String(p.driverName??"").trim()||"—",O=String(p.convoyeurName??"").trim()||"—"}}catch{}}let N=0,k=0,U=0,G=0;if(m&&r&&u&&g&&n)try{const b=se(r,u,g,n),v=await q(M(f,`companies/${a}/agences/${m}/boardingStats/${b}`));if(v.exists()){const p=v.data();N=Number(p.embarkedSeats??0),k=Number(p.absentSeats??0),U=Number(p.totalSeats??p.expectedSeats??p.expectedCount??Math.max(0,N+k)),G=Number(p.reservationsCount??p.totalReservations??p.expectedCount??0)}}catch{}return{tripInstanceId:e.id,agencyId:String(e.agencyId??m??""),date:n,heure:g,departure:r,arrival:u,vehiclePlate:x,driverName:B,convoyeurName:O,reservationsCount:Math.max(0,G),totalSeats:Math.max(0,U),embarkedSeats:Math.max(0,N),absentSeats:Math.max(0,k),statutMetier:"validation_agence_requise"}},[m,a]);o.useEffect(()=>{if(!a||!m||!I){S([]),y(!1);return}y(!0);const e=J(K(f,`companies/${a}/tripInstances`),_("agencyId","==",m),_("statutMetier","==","validation_agence_requise")),r=ee(e,async u=>{const g=u.docs.map(s=>({id:s.id,...s.data()})),n=await Promise.all(g.map(s=>H(s)));n.sort((s,h)=>`${s.date} ${s.heure}`.localeCompare(`${h.date} ${h.heure}`)),S(n),y(!1)},()=>{S([]),y(!1)});return()=>r()},[m,a,I,H]);const R=o.useCallback(async e=>(await te(J(K(f,`companies/${a}/agences/${e.agencyId}/reservations`),_("tripInstanceId","==",e.tripInstanceId)))).docs.map(u=>({id:u.id,...u.data()})),[a]),X=o.useCallback(async e=>{if(!(!a||!$)){if(e.statutMetier!=="validation_agence_requise"){alert("Validation impossible: statut métier incorrect.");return}D(e.tripInstanceId);try{await de(a,e.tripInstanceId,$),alert("Départ validé. Le trajet est maintenant en transit.")}catch(r){alert(r instanceof Error?r.message:"Erreur de validation du départ.")}finally{D(null)}}},[a,$]),z=o.useCallback((e,r)=>{const u=Q(new Date,"EEEE d MMMM yyyy 'à' HH:mm",{locale:ae}),g=r.map((n,s)=>{const h=le(n);return`<tr>
            <td>${s+1}</td>
            <td>${i(n.nomClient||"—")}</td>
            <td>${i(n.telephone||"—")}</td>
...
</body></html>`},[j,C,L]),T=o.useCallback(e=>{const r=Q(new Date,"yyyy-MM-dd HH:mm");return`<!doctype html><html><head><meta charset="utf-8" />
