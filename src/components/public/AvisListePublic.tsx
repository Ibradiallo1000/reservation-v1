...
const AvisListePublic: React.FC<Props> = ({ companyId, primaryColor }) => {
  ...
  // Passe au suivant toutes les 5 secondes
  useEffect(() => {
      ...
      <div className="flex justify-center gap-2 mt-4">
        {avis.map((_, idx) => (
          <button
            ...
            className={`w-3 h-3 rounded-full transition-all ${current === idx ? 'bg-blue-600 scale-110' : 'bg-gray-300'}`}
          />
        ))}
      </div>
    </div>
