...
const ValidationChefAgencePage: React.FC = () => {
  const { user, company } = useAuth() as any;
  const theme = useCompanyTheme(company) || { primary: '#0EA5E9', secondary: '#22D3EE' };
  ...
  const validate = async (rep: ShiftReport) => {
    ...
    await updateDoc(ref, {
      status: 'validated',
      managerValidatedAt: Timestamp.now(),
    });
    await load();
  };
    ...
    <div className="max-w-6xl mx-auto p-4">
      <header className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold">Validations – Chef d’agence</h1>
        </div>
      ...
      {!loading && reports.length === 0 && (
        <div className="text-sm text-gray-600">Aucun rapport en attente de validation du chef d’agence.</div>
      )}
        ...
        {reports.map((rep) => (
          <div key={rep.shiftId} className="rounded-2xl border bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <span className="px-3 py-1 rounded-full text-xs font-semibold" style={{ background:'#DBEAFE', color:'#1D4ED8' }}>
                En attente de validation chef d’agence
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-4">
              ...
              <div className="rounded-xl border p-4 bg-gray-50">
                ...
                <div className="text-2xl font-bold">{formatCurrency(rep.amountTotal)}</div>
              </div>
            </div>
                ...
                rep.byRoute.map((g, i) => (
                  <div key={i} className="mb-3 rounded-lg border overflow-hidden">
                    <div className="px-3 py-2 bg-gray-50 flex items-center justify-between">
                      ...
                      <div className="text-sm"><b>{g.totalBillets}</b> billets • <b>{formatCurrency(g.totalMontant)}</b></div>
                    </div>
                      ...
                      <tbody>
                        {g.trips.map((t, k) => (
                          <tr key={k} className="border-t">
                            ...
                            <td className="px-3 py-2 text-right">{formatCurrency(t.montant)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
            ...
            <div className="mt-4 flex gap-2">
              ...
              >
                <CheckCircle className="h-4 w-4 inline mr-1" /> Valider (archiver dans Historique)
              </button>
            </div>
          </div>
        ))}
...
export default ValidationChefAgencePage;
