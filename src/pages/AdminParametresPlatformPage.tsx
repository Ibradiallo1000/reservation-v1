...
import { Save, Trash2, Upload, Palette, Image as ImageIcon, CheckCircle, AlertCircle } from "lucide-react";
...
const AdminParametresPlatformPage: React.FC = () => {
  ...
  const [pickerFor, setPickerFor] = useState<null | "logo" | "favicon" | "banniere" | "slider">(null);
  const [showColorPicker, setShowColorPicker] = useState<null | "primary" | "secondary">(null);

  useEffect(() => {
    (async () => {
      const snap = await getDoc(SETTINGS_REF);
      if (snap.exists()) setSettings(prev => ({ ...prev, ...(snap.data() as Partial<PlatformSettings>) }));
    })();
  }, []);
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="max-w-5xl mx-auto">
        ...
        <AnimatePresence>
          {message.text && (
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
              className={`p-4 rounded-lg shadow mb-4 ${message.type==="success"?"bg-green-100 text-green-700":"bg-red-100 text-red-700"}`}>
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          ...
          <input className="w-full mb-3 px-4 py-2 border rounded-lg"
            value={settings.accroche} onChange={e=>setSettings(s=>({...s, accroche:e.target.value}))} />
          <input className="w-full px-4 py-2 border rounded-lg"
            value={settings.instructionRecherche} onChange={e=>setSettings(s=>({...s, instructionRecherche:e.target.value}))} />
        </div>

        {/* Couleurs */}
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          <h3 className="text-lg font-semibold mb-4 flex items-center gap-2"><Palette/> Couleurs</h3>
          <div className="flex gap-6">
            {[
              ...
              <div key={c.key}>
                ...
                <div className="h-12 w-12 rounded-full border cursor-pointer"
        ...
        {/* Images */}
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          <h3 className="text-lg font-semibold mb-4 flex items-center gap-2"><ImageIcon/> Images</h3>
          ...
          <div className="mb-4">
            ...
            {settings.logoUrl ? (
              <div className="flex items-center gap-4">
                ...
                <button onClick={()=>setPickerFor("logo")} className="bg-gray-100 px-3 py-1 rounded">Changer</button>
              </div>
            ) : (
              <button onClick={()=>setPickerFor("logo")} className="px-4 py-2 border rounded-lg">Ajouter un logo</button>
            )}
          </div>
          ...
          <div className="mb-4">
            ...
            {settings.faviconUrl ? (
              <div className="flex items-center gap-4">
                ...
                <button onClick={()=>setPickerFor("favicon")} className="bg-gray-100 px-3 py-1 rounded">Changer</button>
              </div>
            ) : (
              <button onClick={()=>setPickerFor("favicon")} className="px-4 py-2 border rounded-lg">Ajouter un favicon</button>
            )}
          </div>
          ...
          <div className="mb-4">
            ...
            {settings.banniereUrl ? (
              <div>
                ...
                <button onClick={()=>setPickerFor("banniere")} className="mt-2 bg-gray-100 px-3 py-1 rounded">Changer</button>
              </div>
            ) : (
              <button onClick={()=>setPickerFor("banniere")} className="px-4 py-2 border rounded-lg">Ajouter une bannière</button>
            )}
          </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {settings.imagesSlider.map((url,i)=>(
                <div key={i} className="relative group">
                  ...
                  <button onClick={()=>removeSlide(i)} className="absolute top-1 right-1 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition">
                    <Trash2 size={14}/>
                  </button>
                </div>
              ))}
        ...
        <button onClick={save} className="bg-orange-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-orange-700 flex items-center gap-2">
        ...
        {pickerFor && (
          <ImageSelectorModal
            title={
              pickerFor==="logo" ? "Choisir un logo" :
              pickerFor==="favicon" ? "Choisir un favicon" :
              pickerFor==="banniere" ? "Choisir une bannière" :
